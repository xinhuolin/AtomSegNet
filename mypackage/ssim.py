...
def ssim(img1, img2, window_size=11, size_average=True):
    ssim, _ = _ssim(img1, img2, window_size, size_average)
    return ssim
...
def ms_ssim(img1, img2, levels=5):
    ...
    if img1.is_cuda:
        weight = weight.cuda(img1.get_device())
        msssim = msssim.cuda(img1.get_device())
        mcs = msssim.cuda(img1.get_device())

    for i in range(levels):
        ssim_map, mcs_map = _ssim(img1, img2, 11, True)
        msssim[i] = ssim_map
        mcs[i] = mcs_map
